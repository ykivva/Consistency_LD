...
class BaselineGraph(TrainableModel):
    ...
    def __init__(
        ...
        for task in self.dest_tasks:
            ...
            transfer_down.name = f"{src_task.name}2{task.name}_down"
            ...
            transfer_up.name = f"{src_task.name}2{task.name}_up"
        ...
        for dest_task in dest_tasks:
            ...
            name_down = f"{src_task.name}2{dest_task.name}_down"
            name_up = f"{src_task.name}2{dest_task.name}_up"
            transfer = UNetTransfer(
                src_task, dest_task,
                block={"up": self.edges_in[name_up], "down": self.edges_out[name_down]}
            )
            self.edge_map[key] = transfer
            try:
                if not lazy:
        ...
        if weights_dir:
            ...
            for key, model in self.edge_map.items():
                ...
                if isinstance(model, UNetTransfer):
                    path_down = f"{weights_dir}/{model.src_task.name}2{model.dest_task.name}_down.pth"
                    path_up = f"{weights_dir}/{model.src_task.name}2{model.dest_task.name}_up.pth"
